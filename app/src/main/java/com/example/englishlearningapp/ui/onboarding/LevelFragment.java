...
public class LevelFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        btnFinish.setOnClickListener(v -> {
            // Chuyển vào màn hình chính (MainActivity)
            if (getActivity() != null) {
    ...
    private void setupSelectionLogic() {
        View.OnClickListener levelClickListener = v -> {
            ...
            // ✅ KHẮC PHỤC LỖI R.color.blue:
            // Nếu R.color.blue bị đỏ, hãy đảm bảo bạn đã định nghĩa nó trong colors.xml
            // Hoặc dùng tạm màu hệ thống: android.R.color.holo_blue_dark
            clickedButton.setStrokeColor(ColorStateList.valueOf(Color.parseColor("#4A6CF7")));
            clickedButton.setStrokeWidth(6);

            selectedLevel = clickedButton.getText().toString();
            btnFinish.setEnabled(true);

            // Đổi màu nút Finish khi được kích hoạt
            btnFinish.setBackgroundTintList(ColorStateList.valueOf(Color.parseColor("#4A6CF7")));
        };
    ...
    private void resetButtons() {
        ...
        for (MaterialButton btn : buttons) {
            if (btn != null) {
                btn.setStrokeColor(ColorStateList.valueOf(grayColor));
